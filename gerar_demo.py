...
function injectAcoesTab() {
  ...
  /* Botão na nav — inserido PRIMEIRO */
  var btn = document.createElement('button');
  btn.className = 'nb';
  btn.id        = 'nb-acoes';
  btn.innerHTML = '<span class="ni">&#127919;</span>Ações';
  btn.onclick   = function() { goTab('acoes', btn); renderAcoes(); };
  bnav.insertBefore(btn, bnav.firstChild);

  /* Ao abrir o dashboard do vendedor, vai direto para Ações */
  var _origBoot = window.bootData;
  window.bootData = function(data) {
    _origBoot(data);
    goTab('acoes', document.getElementById('nb-acoes'));
    renderAcoes();
  };
}
